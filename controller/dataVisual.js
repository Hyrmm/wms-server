...
exports.get_salesInfo = async (req, res) => {
    ...
    if (year) {
        ...
        filter_date_end = `${year}-${month ? month : 12}-31 23:59:00`
    }
    ...
    if (month) {
        ...
        for (let item of sql_result) {
            //销售额
            chartsData.salesData[new Date(item.updata_date).getDate() - 1] += item.price
            ...
            if (clientRank[item.client]) {
                clientRank[item.client] += item.price
            } else {
                clientRank[item.client] = item.price
            }
            ...
            if (categoriesPerCache[item.stock_id]) {
                ...
                let index = categoriesPerTemp.push(
                    {
                        sales: { name: `${item.name}/${item.type}`, value: item.price },
                        cost: { name: `${item.name}/${item.type}`, value: cost * item.amount },
                        profit: { name: `${item.name}/${item.type}`, value: item.price - cost * item.amount }
                    }
                ) - 1
        ...
        for (let item of sql_result) {
            //销售额
            chartsData.salesData[new Date(item.updata_date).getMonth()] += item.price
